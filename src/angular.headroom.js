  ...
  function headroom(HeadroomService) {
    return {
      ...
      link: function ($scope, $element) {
        ...
        //Setup watchers to update headroom object properties on attibute change
        $scope.$watch('tolerance', function (newTolerance) {
          if (newTolerance) {
            headroom.tolerance = newTolerance;
          }
        });
        $scope.$watch('offset', function (newOffset) {
          if (newOffset) {
            headroom.offset = newOffset;
          }
        });
        $scope.$watch('classes', function (newClasses) {
          if (newClasses) {
            headroom.classes = newClasses;
          }
        });
